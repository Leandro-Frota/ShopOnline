    ...
    <main className="flex min-h-screen max-w-screen flex-wrap gap-6 items-center justify-center p-24">
      <div className="min-w-full flex justify-end px-12">
        <div className="cursor-pointer hover:scale-125 transition-all relative">
          ...
          <span className="absolute bottom-1  bg-red-500 flex justify-center items-center rounded-full text-[18px] w-[24px] h-[24px]">0</span>
        </div>
      </div>
      {products.map((p) => {
        return (<div key={p.id} className="rounded-md border-2 border-black
         p-6 flex justify-center flex-col items-center gap-4 max-w-48 min-h-[400px] ">
          ...
          <button className="hover:bg-cyan-700 transition-all bg-cyan-500 text-white p-2 rounded min-w-full">ADICIONAR AO CARRINHO</button>
        </div>)
      })}
    </main>
  );
